...
{
	super(accNo,accNm,accBal,credLimit);
}
public void withdraw(float amount)
{
	float	balance=getAccBal();
	if(amount>(getCredLimit()+balance))
...
{
	return "MMCurrentAccount[account No is" + (getAccNo()) + ",account name is" +(getAccNm())+ " account balance is " + (getAccBal()) +"]";
}
}
